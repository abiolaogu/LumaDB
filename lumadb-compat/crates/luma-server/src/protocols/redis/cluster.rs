...
pub fn execute_cluster_command(
    ...
    if args.len() < 2 {
        return Some(RespValue::Error("ERR wrong number of arguments for 'cluster' command".to_string()));
    }
    
    Some(match args[1].to_uppercase().as_str() {
        "INFO" => cluster_info(),
        "NODES" => cluster_nodes(),
        "SLOTS" => cluster_slots(),
        "MYID" => cluster_myid(),
        "KEYSLOT" => {
            if args.len() < 3 {
                RespValue::Error("ERR wrong number of arguments for CLUSTER KEYSLOT".to_string())
            } else {
                // Simple hash slot calculation
                let slot = crc16_slot(&args[2]);
                RespValue::Integer(slot as i64)
            }
        }
        "COUNTKEYSINSLOT" => {
            RespValue::Integer(0) // No keys tracked per slot in this impl
        }
        "GETKEYSINSLOT" => {
            RespValue::Array(Some(vec![])) // No keys
        }
        "READONLY" => RespValue::SimpleString("OK".to_string()),
        "READWRITE" => RespValue::SimpleString("OK".to_string()),
...
fn cluster_nodes() -> RespValue {
    RespValue::BulkString(Some(
        "0000000000000000000000000000000000000001 127.0.0.1:6379@16379 myself,master - 0 0 1 connected 0-16383\n".to_string()
    ))
}
...
/// Calculate Redis cluster hash slot using CRC16
fn crc16_slot(key: &str) -> u16 {
    // Check for hash tags
    let key_bytes = if let Some(start) = key.find('{') {
